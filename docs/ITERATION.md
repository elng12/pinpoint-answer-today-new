...
## 2026-09-04 #858-#860 索引正文小样试验记录

问题：GSC 的完整窗口显示详情页从“少量收录”继续下滑到“近期页面几乎不抓取”。`#772-#787` 只有 2/16 已收录；`#827-#856` 的 30 页全部未收录，其中 25 页为 `Discovered - currently not indexed`。线上抽查页面均为 HTTP 200、自指 canonical、允许索引且进入 sitemap，因此本轮先验证高度重复正文是否是主要质量阻力，不改技术 SEO。
本轮边界：只让未来 `#858`、`#859`、`#860` 使用 `evidence-v1` 正文；不改 `#857` 及更早页面，不改首页固定 SEO 标题/描述、URL、canonical、sitemap、搜索描述版本或每日发布保护。
修改：Worker 新增严格的正文 canary 分配，只认明确 slug；三页在详情 JSON 和 registry 写入 `contentTemplateVersion=evidence-v1`。页面据此改为直接展示五条真实线索的对应短语和解释，不再显示模板化第一人称猜题、固定错误猜测和固定三卡片经验区。名单外页面仍走原渲染。
验证：正文分配的关闭、命中、未命中、非法模式、空名单和错误 slug 均有回归；试验渲染逐条包含真实线索与对应短语，答案块保持在最后，并明确检查不含 `My first`、`I first`、`first guess` 或 `the trap`。400 条 registry 数据校验、lint、根目录和 Worker 类型检查、Pinpoint 守卫、425 个静态页面完整构建及 Worker dry-run 均通过。

...
| candidate Production closure | candidate promotion creates a unique `main` commit, requires an exact-SHA GitHub deployment whose environment is `Production`, ignores Preview success, and may push one build-trigger marker commit when Production never appears | `.github/workflows/ci.yml`, `scripts/verify-pinpoint-candidate-release.mjs`, `lib/puzzles/vercel-production.shared.mjs` |
...
| reset-window checkpoint alerts | exists, warns when source data is still missing at 15:03/15:06/15:09/15:12/15:15/15:20 Beijing time and when production summary is still stale at 15:06/15:09/15:12/15:15/15:20/15:25; stale-summary alerts include the current stuck point: source fetch, content generation, content validation, publish queue, candidate branch, Vercel, or site summary/cache | `worker/src/index.ts`, `worker/wrangler.toml` |
